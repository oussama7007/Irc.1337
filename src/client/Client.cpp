...
//oadouz
Client::Client(int fd, const std::string &ip) : _fd(fd), _ip(ip), _recvBuffer(), _sendBuffer(), _nickname(), _username(), _passOk(false), _nickSet(false), _userSet(false), _failedPassAttempts(0), _dead(false), _closing(false), _deadReason()
{
	if (_fd < 0)
...
Client::~Client()
{
	...
	if (_fd >= 0)
	{
		if (close(_fd) < 0)
		{
			std::cerr << "Warning: close() failed for client fd=" << _fd << ": " << std::strerror(errno) << "." << std::endl;
		}
		_fd = -1;
	}
}
...
void Client::sendMessage(const std::string &message)
{
	...
	if (_sendBuffer.size() > MAX_SEND_BUFFER || message.size() > MAX_SEND_BUFFER - _sendBuffer.size())
	{
...
bool Client::appendToRecvBuffer(const char *data, std::size_t length)
{
	...
	if (_recvBuffer.size() > MAX_RECV_BUFFER || length > MAX_RECV_BUFFER - _recvBuffer.size())
		return false;
...
bool Client::extractLine(std::string &line)
{
	...
	for (std::size_t i = 0; i < line.size(); ++i)
	{
		...
		{
			std::cerr << "Error: client fd=" << _fd << " sent a forbidden control byte in an IRC message." << std::endl;
